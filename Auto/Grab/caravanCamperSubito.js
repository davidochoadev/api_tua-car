...
export default async function scraperCaravanCamper() {
  ...
  let pages;
  try {
    const [results] = await connection.promise().query("SELECT is_automatic, nome_piattaforma, pages FROM bot_settings WHERE nome_piattaforma = 'caravan_camper_subito'");
