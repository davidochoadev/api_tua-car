...
export default async function scraperManualMotoSubito(number_of_pages) {
  ...
  // * 6. Salviamo gli annunci nel database
   if (annunci.length > 0) {
    ...
    return {
      success: true,
      message: "Nessun annuncio trovato su subito.it per moto",
  ...
  return {
    success: true,
    message: "Ricerca manuale effettuata con successo su subito.it per moto, sono stati inseriti " + annunci.length + " annunci",
    numero_di_pagine: number_of_pages,
