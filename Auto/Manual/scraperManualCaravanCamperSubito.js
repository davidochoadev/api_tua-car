...
export default async function scraperManualCaravanCamperSubito(number_of_pages) {
  if(number_of_pages > MAX_PAGES) {
    number_of_pages = MAX_PAGES;
  }

  console.log(chalk.bgGreen(" 🏁 Starting Scraper Manuale Caravan e Camper su Subito.it 🏁 "));
      ...
      for (const item of listItems) {
        ...
        try {
          ...
          try {
            const [results] = await connection
              .promise()
              .query("SELECT id FROM caravan_camper_subito WHERE urn = ?", [
                urn,
              ]);
   ...
   if (annunci.length > 0) {
    try {
      ...
      await connection.promise().query(
        "UPDATE bot_settings SET last_run = ? WHERE nome_piattaforma = 'caravan_camper_subito'",
        [currentTimestamp]
      );
    ...
    return {
      success: true,
      message: "Nessun annuncio trovato su subito.it per caravan e camper",
  ...
  return {
    success: true,
    message: "Ricerca manuale effettuata con successo su subito.it per caravan e camper, sono stati inseriti " + annunci.length + " annunci",
    numero_di_pagine: number_of_pages,
