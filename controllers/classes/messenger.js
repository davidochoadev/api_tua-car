...
export default class Messenger{
   ...
   async login(page) {
      ...
      await page.screenshot({ path: `./Screens/login_page.png` });
      ...
      await new Promise(r => setTimeout(r, 2000));
      await page.screenshot({ path: './Screens/pswsss.png' });
...
/*       if (!page.isClosed()) {
   ...
   async test(arrayOfLinks) {
      ...
      //Go to marketplace with custom location to find cars;
      await page.goto('https://www.facebook.com/marketplace/category/cars', { waitUntil: 'networkidle2' });
      await page.screenshot({ path: './Screens/first_page.png' });
      // TEST DEL LOGIN
   ...
   async advPageInspect(advPage, element) {
      const messagge = await this.getCurrentMessagges();
      console.log(chalk.bgGreen("Open New Browser Page to get User Data..."));
      console.log(chalk.bgCyan("Get Random Messagge:", messagge));
      await advPage.goto(element.url, { waitUntil: 'networkidle2', timeout: 60000 });
      await advPage.setViewport({ width: 1600, height: 1000 });
      await advPage.screenshot({ path: `./Screens/${element.urn}.png` });
      try {
         await advPage.evaluate(selector => document.querySelector(selector).click(), 'div[aria-label="Message seller"]');
         console.log("Clicked Message seller Button");
         const viewport = await advPage.viewport();
         const centerX = viewport.width / 2;
         const centerY = 530;
         await advPage.mouse.move(centerX, centerY);
         await advPage.waitForTimeout(100); // Optionally, add a small delay between the clicks
         await advPage.mouse.down({ clickCount: r }); // Perform the double click
      ...
      switch (timeOfDay) {
        case "mattina":
          arrayOfResponse = [
            ...
            "Buongiorno! L'annuncio sembra essere quello che sto cercando. È ancora disponibile?",
          ];
          break;
        case "pomeriggio":
          arrayOfResponse = [
            ...
            "Buon pomeriggio! Sono interessato all'annuncio, ma vorrei sapere se è ancora disponibile.",
